...
import type { DemonlistLevel, SimplifiedDemon, RouletteState } from './types';

export function simplifyDemon(demon: DemonlistLevel): SimplifiedDemon {
    // handles youtube.com/watch?v=, youtu.be/ and youtube.com/embed|shorts/ links
    const match = demon.verification_url?.match(
        /(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|shorts\/))([\w-]{11})/
    );
    return {
        name: demon.name,
        // NOTE: the demonlist.org list endpoint only exposes the record `holder`,
        // not a separate level creator/publisher field, so that's used here.
        creator: demon.holder,
        position: demon.placement,
        levelID: demon.ingame_id,
