...
export default defineComponent({
    ...
    setup() {
        ...
        // list_type values supported by the demonlist.org API: main, extended, advanced, unbounded
        type DemonlistType = 'main' | 'extended' | 'advanced' | 'unbounded';

        async function fetchDemons(
            listType: DemonlistType,
            limit: number = 250,
            offset: number = 0
        ): Promise<SimplifiedDemon[]> {
            const response = await fetch(
                `https://api.demonlist.org/level/classic/list?limit=${limit}&offset=${offset}&list_type=${listType}`
            );
            if (response.ok) {
                const json = await response.json();
                return (json.data.levels as any[]).map(simplifyDemon);
            } else {
                return [];
            }
        }
        ...
        async function start() {
            ...
            //     }
            // }
            if (selectedLists.main) demons.push(...(await fetchDemons('main')));
            if (selectedLists.extended) demons.push(...(await fetchDemons('extended')));
            if (selectedLists.legacy) {
                // demonlist.org doesn't have a "legacy" tier like pointercrate did,
                // "advanced" is the closest equivalent (see note below the code)
                demons.push(...(await fetchDemons('advanced')));
            }
            if (useOldList.value) {
